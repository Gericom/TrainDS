...
#define SETTING_TERRAIN_TYPE		2//4
#define SETTING_MAX_HEIGHT_LEVEL	30
#define SETTING_SMOOTHNESS			1//3
#define SETTING_QUANTITY_SEA_LAKES	0
#define SETTING_WATER_BORDERS		0
#define SETTING_VARIETY				0
//#define SETTING_SEED				0

static inline bool HasBit(const uint32_t x, const uint8_t y)
{
	return (x & ((uint32_t)1U << y)) != 0;
}
...
struct HeightMap
{
	...
	int      size_y;     //< MapSizeY()
						 ...
						 */
	inline height_t &height(uint32_t x, uint32_t y)
	{
		return h[x + y * dim_x];
	}
};
...
static height_t TGPGetMaxHeight()
{
	...
	*/
	static const int max_height[5][/*MAX_MAP_SIZE_BITS - MIN_MAP_SIZE_BITS*/12 - 6 + 1] = {
	...
	int max_height_from_table = max_height[/*_settings_game.difficulty.terrain_type*/SETTING_TERRAIN_TYPE][MATH_MIN(/*MapLogX()*/6, /*MapLogY()*/6) - 6/*MIN_MAP_SIZE_BITS*/];
	return I2H(MATH_MIN(max_height_from_table, /*_settings_game.construction.max_heightlevel*/SETTING_MAX_HEIGHT_LEVEL));
}
...
static amplitude_t GetAmplitude(int frequency)
{
	...
	*/
	...
	int smoothness = SETTING_SMOOTHNESS;// _settings_game.game_creation.tgen_smoothness;

	/* Get the table index, and return that value if possible. */
	int index = frequency - MAX_TGP_FREQUENCIES + 7;// lengthof(amplitudes[smoothness]);
	amplitude_t amplitude = amplitudes[smoothness][MATH_MAX(0, index)];
	...
	return MATH_CLAMP((TGPGetMaxHeight() - height_range) / height_range, 0, 1) * amplitude;
}
...
static inline bool AllocHeightMap()
{
	...
	_height_map.size_x = 64;//MapSizeX();
	_height_map.size_y = 64;//MapSizeY();
	...
	_height_map.h = new height_t[_height_map.total_size]; //CallocT<height_t>(_height_map.total_size);
...
{
	delete _height_map.h;//free(_height_map.h);
	_height_map.h = NULL;
}
...
static inline height_t RandomHeight(amplitude_t rMax)
{
	...
	return A2H(/*RandomRange(*/MATH_Rand32(&randcontext, 2 * rMax + 1) - rMax);
}
...
static void HeightMapGenerate()
{
	...
	int start = MATH_MAX(MAX_TGP_FREQUENCIES - (int)MATH_MIN(/*MapLogX()*/6, /*MapLogY()*/6), 0);
	bool first = true;
...
static void HeightMapSineTransform(height_t h_min, height_t h_max)
{
	...
	FOR_ALL_TILES_IN_HEIGHT(h) {
		...
		//case LT_TEMPERATE:
			...
			fheight = 2 * fheight - 1;
			/* Sine transform */
			fheight = sin(fheight * M_PI_2);// (double)FX_FX64C_TO_F32(FX_SinFx64c((fx64c)(fheight * FX64C_PI_2) >> FX32_SHIFT));
...
*/
static void HeightMapCurves(uint32_t level)
{
...
#undef F
	...
	const control_point_list_t curve_maps[] = {
		{ /*lengthof(curve_map_1)*/3, curve_map_1 },
		{ /*lengthof(curve_map_2)*/4, curve_map_2 },
		{ /*lengthof(curve_map_3)*/4, curve_map_3 },
		{ /*lengthof(curve_map_4)*/5, curve_map_4 },
	};

	height_t ht[4];// lengthof(curve_maps)];
	MI_CpuClear16(ht, 4 * 2);//MemSetT(ht, 0, 4);// lengthof(ht));

	/* Set up a grid to choose curve maps based on location; attempt to get a somewhat square grid */
	float factor = FX_FX32_TO_F32(FX_Sqrt(FX_F32_TO_FX32((float)_height_map.size_x / (float)_height_map.size_y)));
	uint32_t sx = MATH_CLAMP((int)(((1 << level) * factor) + 0.5), 1, 128);
	uint32_t sy = MATH_CLAMP((int)(((1 << level) / factor) + 0.5), 1, 128);
	uint8_t *c = new uint8_t[sx * sy];//AllocaM(byte, sx * sy);
	
	MATHRandContext32 randcontext;
	MATH_InitRand32(&randcontext, OS_GetTick());

	for (uint32_t i = 0; i < sx * sy; i++) {
		c[i] = /*Random()*/MATH_Rand32(&randcontext, 0) % 4;// lengthof(curve_maps);
	}
	...
	for (int x = 0; x < _height_map.size_x; x++) {
		...
		for (int y = 0; y < _height_map.size_y; y++) {
			...
			uint32_t corner_a = c[x1 + sx * y1];
			uint32_t corner_b = c[x1 + sx * y2];
			uint32_t corner_c = c[x2 + sx * y1];
			uint32_t corner_d = c[x2 + sx * y2];

			/* Bitmask of which curve maps are chosen, so that we do not bother
			* calculating a curve which won't be used. */
			uint32_t corner_bits = 0;
			...
			*h -= I2H(1);
			...
			for (uint32_t t = 0; t < 4/*lengthof(curve_maps)*/; t++) {
				if (!HasBit(corner_bits, t)) continue;

				bool found = false;
				const control_point_t *cm = curve_maps[t].list;
				for (uint32_t i = 0; i < curve_maps[t].length - 1; i++) {
...
static void HeightMapAdjustWaterLevel(amplitude_t water_percent, height_t h_max_new)
{
	...
	int *hist_buf = new int[h_max - h_min + 1]; //CallocT<int>(h_max - h_min + 1);
	/* Fill histogram */
...
{
	int smallest_size = MATH_MIN(64, 64);//_settings_game.game_creation.map_x, _settings_game.game_creation.map_y);
	const int margin = 4;
	int y, x;
	double max_x;
	double max_y;
	...
	for (y = 0; y <= _height_map.size_y; y++) {
		if (HasBit(water_borders, BORDER_NE)) {
			/* Top right */
			max_x = MATH_ABS((perlin_coast_noise_2D(_height_map.size_y - y, y, 0.9, 53) + 0.25) * 5 + (perlin_coast_noise_2D(y, y, 0.35, 179) + 1) * 12);
			max_x = MATH_MAX((smallest_size * smallest_size / 64) + max_x, (smallest_size * smallest_size / 64) + margin - max_x);
		...
		if (HasBit(water_borders, BORDER_SW)) {
			/* Bottom left */
			max_x = MATH_ABS((perlin_coast_noise_2D(_height_map.size_y - y, y, 0.85, 101) + 0.3) * 6 + (perlin_coast_noise_2D(y, y, 0.45, 67) + 0.75) * 8);
			max_x = MATH_MAX((smallest_size * smallest_size / 64) + max_x, (smallest_size * smallest_size / 64) + margin - max_x);
			if (smallest_size < 8 && max_x > 5) max_x /= 1.5;
	...
	for (x = 0; x <= _height_map.size_x; x++) {
		if (HasBit(water_borders, BORDER_NW)) {
			/* Top left */
			max_y = MATH_ABS((perlin_coast_noise_2D(x, _height_map.size_y / 2, 0.9, 167) + 0.4) * 5 + (perlin_coast_noise_2D(x, _height_map.size_y / 3, 0.4, 211) + 0.7) * 9);
			max_y = MATH_MAX((smallest_size * smallest_size / 64) + max_y, (smallest_size * smallest_size / 64) + margin - max_y);
		...
		if (HasBit(water_borders, BORDER_SE)) {
			/* Bottom right */
			max_y = MATH_ABS((perlin_coast_noise_2D(x, _height_map.size_y / 3, 0.85, 71) + 0.25) * 6 + (perlin_coast_noise_2D(x, _height_map.size_y / 3, 0.35, 193) + 0.75) * 12);
			max_y = MATH_MAX((smallest_size * smallest_size / 64) + max_y, (smallest_size * smallest_size / 64) + margin - max_y);
			if (smallest_size < 8 && max_y > 5) max_y /= 1.5;
...
static void HeightMapSmoothCoastInDirection(int org_x, int org_y, int dir_x, int dir_y)
{
	...
	* Soften the coast slope */
	for (depth = 0; IsValidXY(x, y) && depth <= max_coast_Smooth_depth; depth++, x += dir_x, y += dir_y) {
		...
		h = MATH_MIN(h, h_prev + (4 + depth)); // coast softening formula
...
static void HeightMapSmoothSlopes(height_t dh_max)
{
	for (int y = 0; y <= (int)_height_map.size_y; y++) {
		for (int x = 0; x <= (int)_height_map.size_x; x++) {
			height_t h_max = MATH_MIN(_height_map.height(x > 0 ? x - 1 : x, y), _height_map.height(x, y > 0 ? y - 1 : y)) + dh_max;
	...
	for (int y = _height_map.size_y; y >= 0; y--) {
		for (int x = _height_map.size_x; x >= 0; x--) {
			height_t h_max = MATH_MIN(_height_map.height(x < _height_map.size_x ? x + 1 : x, y), _height_map.height(x, y < _height_map.size_y ? y + 1 : y)) + dh_max;
			if (_height_map.height(x, y) > h_max) _height_map.height(x, y) = h_max;
		}
	}
}
...
{
	int sea_level_setting = SETTING_QUANTITY_SEA_LAKES;// _settings_game.difficulty.quantity_sea_lakes;
	...
	const height_t roughness = 7 + 3 * SETTING_SMOOTHNESS;//_settings_game.game_creation.tgen_smoothness;

	HeightMapAdjustWaterLevel(water_percent, h_max_new);

	uint8_t water_borders = /*_settings_game.construction.freeform_edges*/TRUE ? /*_settings_game.game_creation.water_borders*/SETTING_WATER_BORDERS : 0xF;
	...
	if (SETTING_VARIETY > 0) {
		HeightMapCurves(SETTING_VARIETY);
	}
...
{
	long n = x + y * prime + 0;//_settings_game.game_creation.generation_seed;
...
static void TgenSetTileHeight(tile_t* tile, int height)
{
	tile->y = height;
...
*/
void GenerateTerrainPerlin(Map* map)
{
	...
	if (/*_settings_game.construction.freeform_edges*/TRUE) {
	...
	for (int y = 0; y < _height_map.size_y; y++) {
		for (int x = 0; x < _height_map.size_x; x++) {
			TgenSetTileHeight(&map->mTiles[y][x], MATH_CLAMP(H2I(_height_map.height(x, y)), 0, max_height));
		}
	}
